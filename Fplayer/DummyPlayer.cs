...
internal class DummyPlayer
{
    ...
    private const float GROUND_Y = 1000f; // 地面高度（临时值，后续会从世界数据获取）
    ...
    {
        // 设置初始位置
        _position = new Microsoft.Xna.Framework.Vector2(x * 16f, y * 16f); // 转换为像素坐标
    ...
    public void Move(string direction)
    {
        ...
        {
            case "left":
                velocity = new Microsoft.Xna.Framework.Vector2(-3f, 0f);
                flag1 = 0b00000001; // 移动标志
                animationType = 1; // 行走动画
                break;
            case "right":
                velocity = new Microsoft.Xna.Framework.Vector2(3f, 0f);
                flag1 = 0b00000001; // 移动标志
        ...
        // 更新位置和速度
        _velocity = velocity;
        _position += velocity; // 简单的位置更新
        
        // 确保玩家处于活跃状态
        SendPlayerActive(true);
        // 发送动画数据包
        SendPlayerAnimation(animationType);
        // 发送移动数据包
        SendPlayerUpdate(flag1, 0, 0, 0, 0, velocity);
    }
